...
void glib_init(void) {
  ...
  {
    const char *glib_err = glib_check_version(
...
GValue *g_value_unset_copy(const GValue *from, GValue *to) {
  ...
  return to;
     ...
     (GLIB_MAJOR_VERSION == 2 && GLIB_MINOR_VERSION < 28))
...
static gboolean g_value_set_boolean_from_string(GValue *val, char *str) {
...
static char **g_flags_to_strv(int value, GType type, FlagString source) {
  ...
  for (flagsvalue = class->values; flagsvalue->value_name != NULL;
       flagsvalue++) {
    if (source == FLAG_STRING_SHORT_NAME) {
      common_prefix = find_common_prefix(common_prefix, flagsvalue->value_name);
    }
...
guint g_str_amanda_hash(gconstpointer key) {
